...
export function loadRefreshToken(accountId) {
...
export function storeRefreshToken(accountId, refreshToken) {
    const path = tokenPath(accountId);
...
export function deleteRefreshToken(accountId) {
    const path = tokenPath(accountId);
    try {
        if (existsSync(path))
            unlinkSync(path);
