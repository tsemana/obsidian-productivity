...
export async function refreshAccessToken(refreshToken, clientId, clientSecret) {
    ...
    if (!res.ok) {
        ...
        if (res.status === 400 && body.includes("invalid_grant")) {
            throw new Error("Refresh token has been revoked. Run account_register again to re-authorize.");
        }
 ...
 */
export async function authorizeAccount(email, clientId, clientSecret) {
    const state = randomBytes(16).toString("hex");
    const { code, port, server } = await new Promise((resolve, reject) => {
        let boundPort;
        let resolved = false;
    ...
    if (!tokens.refresh_token) {
        throw new Error("No refresh token received. This can happen if the account was previously authorized. " +
