...
function requireVault() {
    if (!vaultPath) {
        throw new Error("No vault path configured. Set OBSIDIAN_VAULT_PATH environment variable, " +
            "pass the vault path as a command-line argument, or run from inside the vault directory.");
    }
    return vaultPath;
}
// ─── Group 1: Vault Management ─────────────────────────────────────────────
server.tool("vault_init", "Create missing vault directories (tasks/, daily/, memory/, templates/, bases/, canvas/ and subdirectories). Returns what was created vs what already existed.", { directories: z.array(z.string()).optional().describe("Specific directories to create. Default: full standard set.") }, async ({ directories }) => ({
    content: [{ type: "text", text: JSON.stringify(vaultInit(requireVault(), directories), null, 2) }],
}));
server.tool("vault_health", "Check vault state: verify expected directories exist, count notes by type, check for CLAUDE.md, check .obsidian/ config presence.", {}, async () => ({
    content: [{ type: "text", text: JSON.stringify(vaultHealth(requireVault()), null, 2) }],
}));
server.tool("vault_list", "List files in a vault directory. Returns filenames with optional frontmatter summary.", {
    ...
    content: [{ type: "text", text: JSON.stringify(vaultList(requireVault(), directory, { include_frontmatter, recursive, extension }), null, 2) }],
}));
// ─── Group 2: Task Operations ──────────────────────────────────────────────
server.tool("task_create", "Create a new task note in tasks/ with frontmatter and optional body content. Enforces the task schema (title, tags:[task], status, priority, created).", {
...
server.tool("task_update", "Update an existing task note. Can modify frontmatter fields, append to body, or replace body sections. Merges with existing frontmatter.", {
...
server.tool("task_list", "List task notes with frontmatter, optionally filtered by status, priority, context, project, or due date range.", {
...
server.tool("memory_read", "Read a memory file (person, project, glossary, company context) by path or by searching for a name/alias.", {
...
server.tool("memory_write", "Create or update a memory file (person, project, glossary entry, company context). For glossary.md, can append entries to specific tables.", {
...
server.tool("claudemd_read", "Read the CLAUDE.md file from the vault root. This is the hot-cache working memory file.", {}, async () => ({
    content: [{ type: "text", text: JSON.stringify(claudemdRead(requireVault()), null, 2) }],
}));
server.tool("claudemd_update", "Update CLAUDE.md. Can replace a specific section by heading, append content, or write the entire file.", {
...
server.tool("note_read", "Read any file in the vault. Returns frontmatter (if present) and body content. Works with .md, .base, .canvas, and .json files.", {
...
server.tool("note_write", "Write a file to the vault. For markdown files, accepts separate frontmatter and body. For other formats, accepts raw content. Creates parent directories if needed.", {
    path: z.string().describe("Relative path from vault root"),
...
server.tool("note_update", "Generic patch-style update for an existing note or file. For markdown files, can merge frontmatter, append body text, replace a section, or replace the full body. For non-markdown files, use raw content.", {
    path: z.string().describe("Relative path from vault root"),
...
server.tool("note_search", "Search vault notes by text content and/or frontmatter properties. Returns matching files with context.", {
...
server.tool("note_move", "Move a file within the vault. Used primarily for archiving tasks to tasks/done/.", {
...
server.tool("wikilink_consolidate", "Consolidate all wikilinks for a given note to the canonical [[filename-slug|Display Name]] format. Finds the target note, collects its aliases, and rewrites all variant links across the vault.", {
    ...
    content: [{ type: "text", text: JSON.stringify(wikilinkConsolidate(requireVault(), name, dry_run, db ?? undefined), null, 2) }],
}));
server.tool("wikilink_validate", "Scan the vault for broken wikilinks — links whose targets don't exist as files or aliases. Returns broken links with suggested corrections.", {
    ...
    content: [{ type: "text", text: JSON.stringify(wikilinkValidate(requireVault(), directory, fix_suggestions, db ?? undefined), null, 2) }],
}));
// ─── Group 6: Bases & Canvas ───────────────────────────────────────────────
server.tool("base_read", "Read and parse an Obsidian .base file (YAML format). Returns the parsed structure as JSON.", {
    path: z.string().describe("Relative path to .base file"),
}, async ({ path }) => ({
    content: [{ type: "text", text: JSON.stringify(baseRead(requireVault(), path), null, 2) }],
}));
server.tool("base_write", "Write an Obsidian .base file. Accepts the base definition as a structured object and serializes to YAML.", {
...
server.tool("canvas_read", "Read and parse an Obsidian .canvas file (JSON format). Returns nodes and edges.", {
...
server.tool("canvas_write", "Write an Obsidian .canvas file. Validates node/edge structure before writing.", {
    ...
    content: [{ type: "text", text: JSON.stringify(canvasWrite(requireVault(), path, nodes, edges), null, 2) }],
}));
// ─── Group 7: Obsidian Config ──────────────────────────────────────────────
server.tool("obsidian_config_read", "Read an Obsidian config file from .obsidian/ directory. Returns parsed JSON.", {
...
server.tool("obsidian_config_write", "Write or update an Obsidian config file in .obsidian/. Creates .obsidian/ directory if needed. Merges with existing content by default.", {
...
server.tool("weekly_review", "Gather all data for the GTD Weekly Review in a single call. Returns inbox items, active tasks, waiting-fors with days waiting, project summaries, someday/maybe, calendar (2 weeks forward + 1 week back), and memory reference frequency.", {}, async () => {
    if (!db)
        return { content: [{ type: "text", text: JSON.stringify({ error: "no_database", message: "SQLite not initialized" }) }] };
    const result = await weeklyReview(db, requireVault());
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
});
server.tool("project_overview", "Get a holistic view of a single project: project note, linked tasks by status, connected people, recent FTS5 mentions, and wikilink graph neighbors.", {
...
server.tool("quick_capture", "Capture a thought — creates a structured task (hint='task') or a raw inbox note (hint='idea'/'reference'/'unknown'). Returns suggested wikilinks from FTS5.", {
...
server.tool("search_and_summarize", "Full-text search with BM25 ranking and context snippets. Supports boolean operators, phrase queries, and prefix matching. Logs results for reference frequency tracking.", {
