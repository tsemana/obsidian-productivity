...
function extractNoteRow(filePath, content, mtime) {
...
export function fullScan(db, vaultPath) {
...
export function incrementalSync(db, vaultPath) {
    const diskFiles = walkVault(vaultPath);
    const diskMap = new Map(diskFiles.map((f) => [f.path, f.mtime]));
    // Get all indexed paths and mtimes
    const indexed = db.prepare("SELECT path, modified_at, content_hash FROM notes").all();
...
export function runSync(db, vaultPath) {
