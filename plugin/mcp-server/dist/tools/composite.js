...
export async function radarData(db, vaultPath, options = {}) {
  ...
  `).all();
    ...
    if (!calendarAvailable && !includeCalendar) {
        try {
            calendarAvailable = db.prepare("SELECT COUNT(*) as cnt FROM calendar_events").get().cnt > 0;
        }
        catch { }
    }
    if (!emailAvailable && !includeEmail) {
        try {
            emailAvailable = db.prepare("SELECT COUNT(*) as cnt FROM email_cache").get().cnt > 0;
        }
        catch { }
    }
    return {
...
export async function projectOverview(db, vaultPath, options) {
  ...
  `).all(slug);
    ...
    LIMIT 20
  `).all(slug, thirtyDaysAgo, thirtyDaysAgo);
