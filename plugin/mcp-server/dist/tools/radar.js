...
function renderDailyNote(date, data) {
    ...
    lines.push(``, `## Calendar`, calLines.length > 0 ? calLines.join("\n") : "_No events today_", ``, `## Open Loops`, loopItems.length > 0 ? loopItems.join("\n") : "_All clear_", ``, `## Email Highlights`, emailLines.length > 0 ? emailLines.join("\n") : "_No emails_");
    return lines.join("\n");
}
/** Write or update daily note in daily/YYYY-MM-DD.md */
function writeDailyNote(vaultPath, date, data) {
...
export async function radarGenerate(db, vaultPath, options = {}) {
    ...
    const overdueTasks = data.vault.tasks.overdue.map((t) => ({
        path: t.path,
        title: t.title,
        priority: t.priority,
        due: t.due,
    ...
    const activeTasks = data.vault.tasks.active.map((t) => ({
        path: t.path,
        title: t.title,
        priority: t.priority,
        due: t.due,
    ...
    const waitingTasks = data.vault.tasks.waiting.map((t) => ({
        path: t.path,
        title: t.title,
        priority: t.priority,
        due: t.due,
...
export function radarUpdateItem(vaultPath, options) {
    const date = options.date ?? todayStr();
...
function eventHtml(event) {
    ...
    const badges = [];
    if (event.rsvp_status === "needsAction")
        badges.push('<span class="badge badge-yellow">RSVP</span>');
    if (event.rsvp_status === "declined")
        badges.push('<span class="badge badge-gray">DECLINED</span>');
...
function loopItemHtml(task, dotClass) {
...
function buildFireItems(overdue, emails, active, date) {
    const items = [];
...
function buildWatchItems(active, waiting, events, date) {
    const items = [];
...
function getEventCssClass(event) {
    ...
    if (event.rsvp_status === "needsAction")
        return "needs-rsvp";
    if (event.context === "personal")
        return "personal";
    const titleLower = (event.title ?? "").toLowerCase();
    if (titleLower.includes("focus") || titleLower.includes("block"))
        return "focus";
    return "";
}
