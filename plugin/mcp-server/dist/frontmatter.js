...
export function parseNote(content) {
    try {
        ...
        const fm = result.data && Object.keys(result.data).length > 0
            ? result.data
            : null;
        return {
            frontmatter: fm,
 ...
 */
export function replaceSection(body, heading, newContent) {
    ...
    const headingRegex = new RegExp(`^${escapeRegex(fullHeading)}\\s*$`, "m");
 ...
 */
export function matchesFrontmatter(frontmatter, filter) {
    if (!frontmatter)
        return false;
    for (const [key, expected] of Object.entries(filter)) {
        ...
        if (Array.isArray(actual)) {
            ...
            if (Array.isArray(expected)) {
                ...
                if (!expected.some((v) => actual.includes(v)))
                    return false;
            }
            else {
